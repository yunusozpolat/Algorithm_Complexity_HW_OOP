...
public class Galli {
    ...
    public void Add(Galli newGalli) {
        String s = Integer.toString(newGalli.Currency);
        ...
        for(int i = s1.length() -1  ; i >= 0; i-- ){
            number1 = Integer.parseInt(sArray1[i]) * (int)Math.pow(9,index1);
    ...
    public void Add(Kuba newKuba) {
        int temp = Currency;
        String s = Integer.toString(newKuba.TakeCurrency(temp));
        String[] sArray = s.split("");
        int[] sArray2 = new int[s.length()];
        int number = 0;
        int index  = 0;
    ...
    public void GetCurrency() {
        ...
        while (true) {
            ...
            System.out.print("Enter a galli currency: ");
            ...
            for (int i = 0; i < kubaCurrency.length(); i++) {
                if (kubaCurrency.length() > 9 || currencyArray[i].equals("9")) {
                    System.out.println("Error: Icorrect galli currency.Please enter new galli currency");
                    isFeasibleKuba = false;
                    break;
                }
            }
            if (isFeasibleKuba) {
                this.Currency = kubaInput;
                break;
            }
        }
    }
    public void GetCurrency(Integer newGalli) {
        String newKubacurrency = Integer.toString(newGalli);
        String[] newKubaCurrency = newKubacurrency.split("");
        for (int i = 0; i < newKubacurrency.length(); i++) {
            if (newKubacurrency.length() > 9 || newKubaCurrency[i].equals("9")) {
                System.out.println("Error: Icorrect galli currency. Please enter new galli currency");
                break;
            }
        }
    ...
    public Kuba Convert() {
        ...
        int kuba = sum1 * 2;
        String stringCurrency = Integer.toString(kuba);
        int division = kuba / 7;
        int mod = kuba % 7;
